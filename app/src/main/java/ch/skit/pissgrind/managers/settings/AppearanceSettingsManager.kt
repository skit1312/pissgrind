package ch.skit.pissgrind.managers.settings
...
import ch.skit.pissgrind.dataStore
import ch.skit.pissgrind.ui.playing.NowPlayingAlignment
import ch.skit.pissgrind.ui.playing.NowPlayingBackground
import ch.skit.pissgrind.ui.screens.HomeItem
...
class AppearanceSettingsManager @Inject constructor(
    ...
    val bottomNavItemsFlow: Flow<List<BottomNavItem>> = context.dataStore.data.map { preferences ->
        ...
        val defaultValue = listOf(
            ...
            BottomNavItem("Request", R.drawable.rounded_add_24, "radio_screen")

        )
        try {
            val items = jsonString?.let { Json.decodeFromString<List<BottomNavItem>>(it) } ?: defaultValue
            // Ensure radio_screen (Request) is enabled
            items.map { item ->
                if (item.screenRoute == "radio_screen") item.copy(title = "Request", icon = R.drawable.rounded_add_24, enabled = true)
                else item
            }.let { list ->
                if (list.none { it.screenRoute == "radio_screen" }) list + BottomNavItem("Request", R.drawable.rounded_add_24, "radio_screen")
                else list
            }
        } catch (e: Exception) {
            println(e.message)
