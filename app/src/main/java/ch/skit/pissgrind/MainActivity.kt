...
import ch.skit.pissgrind.data.BottomNavItem
...
import ch.skit.pissgrind.player.ChoraMediaLibraryService
import ch.skit.pissgrind.player.rememberManagedMediaController
import ch.skit.pissgrind.ui.elements.dialogs.tv.OnboardingDialog
import ch.skit.pissgrind.ui.playing.NowPlayingContent
import ch.skit.pissgrind.ui.playing.NowPlayingMiniPlayer
import ch.skit.pissgrind.ui.playing.dpToPx
import ch.skit.pissgrind.ui.theme.MusicPlayerTheme
...
import ch.skit.pissgrind.ui.theme.PissBlack
import ch.skit.pissgrind.ui.theme.PissGreen
...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            val showRegisteredPopup = mutableStateOf(false)
            
            // check intent
            if (intent?.data?.scheme == "pissgrind" && intent?.data?.host == "registered") {
                showRegisteredPopup.value = true
            }
                // Registration Popup
                if (showRegisteredPopup.value) {
                    AlertDialog(
                        onDismissRequest = { showRegisteredPopup.value = false },
                        confirmButton = {
                            TextButton(onClick = { showRegisteredPopup.value = false }) {
                                Text("OK")
                            }
                        },
                        title = { Text("Registration submitted! ✓") },
                        text = { Text("Your account will be reviewed and approved shortly. You'll receive an email once it's activated.") }
                    )
                }
            ...
            MusicPlayerTheme (darkTheme) {
                ...
                if (!showNoProvidersDialog) {
                    if (isTv) {
                        ...
                        ch.skit.pissgrind.ui.elements.dialogs.OnboardingDialog() { showNoProvidersDialog = true }
            ...
            Column(
                ...
                orderedNavItems.forEach { item ->
                    ...
                    val icon = when (item.screenRoute) {
                        ...
                        "radio_screen"   -> R.drawable.rounded_add_24
...
fun AnimatedBottomNavBar(
    ...
    val orderedNavItems = AppearanceSettingsManager(context).bottomNavItemsFlow.collectAsState(
        initial = listOf(
            BottomNavItem(
                ...
                "Request", R.drawable.rounded_add_24, "radio_screen"
            )
    ...
    if (LocalConfiguration.current.orientation == Configuration.ORIENTATION_PORTRAIT) {
        ...
        Column {
            HorizontalDivider(color = PissGreen, thickness = 1.dp)
            NavigationBar(containerColor = PissBlack, modifier = Modifier.offset { IntOffset(x = 0, y = -yTrans) }) {
